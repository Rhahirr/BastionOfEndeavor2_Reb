	...
	switch(choice)
		if("Общий совет")
			...
			return pick(
				...
				prob(60); "[info]Наша вики активно обновляется! Она доступна по адресу [config.wikiurl]",
				...
				prob(50); "[info]В рамках поддержания веселой и комфортной атмосферы для всех игроков, у нас применяется политика согласия свидетелей: учитывайте предпочтения тех, кто находится рядом! [config.wikiurl]Правила#Политика_согласия_свидетелей",
				...
				prob(30); "[info]Не знаете, с чего начать? Нажмите английскую T и напишите: '; Привет, я новый работник! Кто-нибудь может показать мне станцию?' или что-то похожее! Больше советов для новичков здесь: [config.wikiurl]Черты",
		...
		if("gameplay")
			...
			return pick(
				...
				prob(25); "[info]В Space Station 13 есть два режима управления – с горячими клавишами и без. Их можно переключить с помощью TAB. В режиме горячих клавиш вы можете говорить при нажатии английской T, и у вас будет отображаться маленькое облачко речи. В режиме без горячих клавиш вы можете писать глаголы (команды) вручную в строке снизу. Справка по управлению в обоих режимах: [config.wikiurl]Управление (или в меню Помощь > Показать управление).",
		...
		if("Ролевой отыгрыш")
			...
			return pick(
				...
				prob(25); "[info]Несмотря на то, что Стандарту Операционной Деятельности не обязательно следовать, и что он может быть нарушен (однако, с возможными внутриигровыми последствиями для вашего персонажа), службе безопасности и медицинскому отделу все равно настоятельно рекомендуется с ним ознакомиться. [config.wikiurl]Стандарт_операционной_деятельности",
		...
		if("Сеттинг и лор")
			...
			return pick(
				...
				prob(75); "[info]Ключевые моменты нашего сеттинга можно прочитать здесь: [config.wikiurl]Предыстория",
				prob(50); "[info]Вы работаете в системе Вир на объекте ЛС-НТ Южный Крест, станции телекоммуникаций и контроля трафика. [config.wikiurl]Вир",
				prob(50); "[info]Большая часть экипажа проживает в колонии астероидов Северная Звезда, вращающейся вокруг газового гиганта под названием Кара. Она же является хабом для посетителей, центром шахтёрской деятельности и логистики. Именно туда вас отвозит шаттл в конце каждого раунда. На шахты можно попасть с помощью экспедиционного шаттла. [config.wikiurl]ЦС-НТ_Северная_Звезда",
				prob(10); "[info]Универсальной валютой является талер. Он поддерживается Центральным Солом и всеми его союзниками. Несмотря на его избыток на планетах у границы, талер очень невыгодно обменивать на валюты более развитых и устоявшихся регионов, что, например, сильно ограничивает миграцию в такие места, как планета Земля. [config.wikiurl]Предыстория"
			)
	// End of Bastion of Endeavor Translation
