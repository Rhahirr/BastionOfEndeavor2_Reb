...
/client/proc/browse_files(root="data/logs/", max_iterations=10, list/valid_extensions=list(".txt",".log",".htm"))
	...
	if( !fexists(path) || !(extension in valid_extensions) )
		...
		*/
		to_chat(src, span_red("Ошибка browse_files(): Файл не найден или не является допустимым ([path])."))
		// End of Bastion of Endeavor Translation
		return
...
/client/proc/file_spam_check()
	...
	if(time_to_wait > 0)
		...
		*/
		to_chat(src, span_red("Ошибка file_spam_check(): Зафиксирован спам. Пожалуйста, подождите [count_ru(round(time_to_wait/10), "секунд;у;ы;")]."))
		// End of Bastion of Endeavor Translation
		return 1
